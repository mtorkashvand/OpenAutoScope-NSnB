...
class TrackerDevice():
    ...
    def process(self):
        ...
        mask = np.zeros_like(dsimg) # Create mask where white is what we want, black otherwise
        cv2.drawContours(mask, contours, max_size_idx, 255, -1) # Draw filled contour in mask
        cropped_img = np.zeros_like(dsimg) # Extract out the object and place into output image
        cropped_img[mask == 255] = dsimg[mask == 255]
    ...
    def change_threshold(self, direction):
        ...
        print("Threshold: {}     ".format(self.threshold), end="\r")
